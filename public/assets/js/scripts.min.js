!function(modules){function __webpack_require__(moduleId){if(installedModules[moduleId])return installedModules[moduleId].exports;var module=installedModules[moduleId]={exports:{},id:moduleId,loaded:!1};return modules[moduleId].call(module.exports,module,module.exports,__webpack_require__),module.loaded=!0,module.exports}var installedModules={};return __webpack_require__.m=modules,__webpack_require__.c=installedModules,__webpack_require__.p="",__webpack_require__(0)}([function(module,exports,__webpack_require__){__webpack_require__(20),__webpack_require__(17),__webpack_require__(18),__webpack_require__(19),__webpack_require__(21),__webpack_require__(7),__webpack_require__(8),__webpack_require__(6),__webpack_require__(5),__webpack_require__(4),__webpack_require__(10),__webpack_require__(9),__webpack_require__(1),__webpack_require__(1),__webpack_require__(15),__webpack_require__(16),__webpack_require__(14),__webpack_require__(13),__webpack_require__(11),__webpack_require__(12),__webpack_require__(2),__webpack_require__(3)},function(module,exports){angular.module("bookService",[]).factory("Book",["$q","$http",function($q,$http){function getAllBooks(){var deffered=$q.defer();return $http.get("/api/all-books").success(function(data){data&&deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}function getUserBooks(){var deffered=$q.defer();return $http.get("/api/user-books").success(function(data){data&&deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}function addUserBook(title){var deffered=$q.defer();return $http.post("/api/user-books",{title:title}).success(function(data){data&&deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}function deleteUserBook(id){var deffered=$q.defer();return $http["delete"]("/api/user-books/"+id).success(function(data){data&&deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}var bookFactory={getAllBooks:getAllBooks,getUserBooks:getUserBooks,addUserBook:addUserBook,deleteUserBook:deleteUserBook};return bookFactory}])},function(module,exports){angular.module("appRoutes",["ui.router"]).config(["$stateProvider","$urlRouterProvider","$locationProvider",function($stateProvider,$urlRouterProvider,$locationProvider){function skipIfLoggedIn($q,$auth){var deferred=$q.defer();return $auth.isAuthenticated()?deferred.reject():deferred.resolve(),deferred.promise}function loginRequired($q,$location,$auth){var deffered=$q.defer();return $auth.isAuthenticated()?deffered.resolve():$location.path("/"),deffered.promise}$stateProvider.state("auth",{url:"/",views:{"":{templateUrl:"app/views/pages/auth.html",controller:"authController as authenticate"},resolve:{skipIfLoggedIn:["$q","$auth",skipIfLoggedIn]}}}).state("home",{url:"/home",views:{"":{templateUrl:"app/views/pages/home.html",controller:"libraryController as library"},"nav@home":{templateUrl:"app/views/partials/navbar.html",controller:"navbarController as navbar"}},resolve:{loginRequired:["$q","$location","$auth",loginRequired]}}).state("account",{url:"/account",views:{"":{templateUrl:"app/views/pages/account.html",controller:"accountController as account"},"nav@account":{templateUrl:"app/views/partials/navbar.html",controller:"navbarController as navbar"}},resolve:{loginRequired:["$q","$location","$auth",loginRequired]}}).state("inbox",{url:"/inbox",views:{"":{templateUrl:"app/views/pages/inbox.html",controller:"inboxController as inbox"},"nav@inbox":{templateUrl:"app/views/partials/navbar.html",controller:"navbarController as navbar"}},resolve:{loginRequired:["$q","$location","$auth",loginRequired]}}).state("your-books",{url:"/your-books",views:{"":{templateUrl:"app/views/pages/my-books.html",controller:"yourBooksController as books"},"nav@your-books":{templateUrl:"app/views/partials/navbar.html",controller:"navbarController as navbar"}},resolve:{loginRequired:["$q","$location","$auth",loginRequired]}}).state("user",{url:"/user/:id?:book_id",views:{"":{templateUrl:"app/views/pages/user.html",controller:"usersController as user"},"nav@user":{templateUrl:"app/views/partials/navbar.html",controller:"navbarController as navbar"}},resolve:{loginRequired:["$q","$location","$auth",loginRequired]}}),$urlRouterProvider.otherwise("/home"),$locationProvider.html5Mode(!0)}])},function(module,exports){angular.module("mainApp",["ui.router","satellizer","ui.gravatar","ngMessages","libraryCtrl","yourBooksCtrl","inboxCtrl","authCtrl","navbarCtrl","accountCtrl","usersCtrl","navbarButtonDirective","bookDirective","modalDialogDirective","bookService","inboxService","usersService","accountService","appRoutes"])},function(module,exports){angular.module("accountCtrl",["satellizer","accountService"]).controller("accountController",["$auth","Account","$location",function($auth,Account,$location){var vm=this;vm.accountCreds={},vm.accountInfo={},vm.getAccount=function(){Account.getAccountInfo().then(function(data){vm.accountCreds.name=data[0].name,vm.accountCreds.email=data[0].email,vm.accountCreds.location=data[0].location})["catch"](function(data){console.log("error...")})},vm.updateAccountInfo=function(){Account.updateInfo(vm.accountInfo.name,vm.accountInfo.email,vm.accountInfo.location,vm.accountInfo.password).then(function(data){vm.accountInfo={},vm.getAccount()})["catch"](function(data){console.log("error ....")})},vm.deleteAccount=function(){Account.deleteAccount().then(function(){$auth.logout().then(function(){$location.path("/")})})["catch"](function(){console.log("error...")})},vm.getAccount()}])},function(module,exports){angular.module("authCtrl",["satellizer"]).controller("authController",["$location","$auth",function($location,$auth){var vm=this,stateSelection=["login","signup"];vm.selection=stateSelection[0],vm.login=function(){$auth.login({email:vm.user.email,password:vm.user.password}).then(function(res){res.data.token?$location.path("/home"):(vm.error=!0,vm.errorMessage=res.data.message)})["catch"](function(data){vm.error=!0,vm.errorMessage="Failed to login, please try again"})},vm.signup=function(){vm.signupForm.$valid&&$auth.signup({name:vm.user.name,email:vm.user.email,password:vm.user.password}).then(function(res){res.data.token?($auth.setToken(res.data.token),$location.path("/portfolio")):(vm.error=!0,vm.errorMessage=res.data.message)})["catch"](function(){vm.error=!0,vm.errorMessage="Failed to sign up, please try again"})},vm.select=function(index){vm.selection=stateSelection[index]}}])},function(module,exports){angular.module("inboxCtrl",["inboxService"]).controller("inboxController",["Inbox",function(Inbox){function filterRequests(bookRequests){bookRequests.filter(function(request){request.isUsersRequest===!0?vm.bookRequestsForUser.push(request):vm.outstandingBookRequests.push(request)})}function deleteBookRequest(arr,index){var requestArray=arr,bookRequestId=requestArray[index]._id;Inbox.deleteRequest(bookRequestId).then(function(data){vm.outstandingBookRequests=[],vm.bookRequestsForUser=[],vm.getRequests()})["catch"](function(data){console.log(data.err)})}var vm=this;vm.noRequests=!1,vm.noRequestMessage="There are no book swap requests.",vm.bookRequestsForUser=[],vm.outstandingBookRequests=[],vm.getRequests=function(){Inbox.getRequests().then(function(data){return data[0].bookRequests.length?void filterRequests(data[0].bookRequests):void(vm.noRequests=!0)})["catch"](function(data){console.log("error....")})},vm.approve=function(index){var bookRequestId=vm.bookRequestsForUser[index]._id;Inbox.setApproval(bookRequestId).then(function(data){vm.bookRequestsForUser=[],vm.getRequests()})["catch"](function(data){console.log("error..."+err)})},vm.deleteRequestForUser=function(index){deleteBookRequest(vm.bookRequestsForUser,index)},vm.deleteOutstandingRequest=function(index){deleteBookRequest(vm.outstandingBookRequests,index)},vm.getRequests()}])},function(module,exports){angular.module("libraryCtrl",["bookService"]).controller("libraryController",["Book",function(Book){var vm=this;vm.libraryBooks=[],vm.getBooks=function(){Book.getAllBooks().then(function(data){vm.libraryBooks=data})["catch"](function(data){console.log("error .....")})},vm.getBooks()}])},function(module,exports){angular.module("yourBooksCtrl",["bookService"]).controller("yourBooksController",["Book",function(Book){var vm=this;vm.yourBooks=[],vm.getBooks=function(){Book.getUserBooks().then(function(data){vm.yourBooks=data.books})["catch"](function(data){console.log("error fetching books")})},vm.addBook=function(){""!==vm.book.title&&Book.addUserBook(vm.book.title).then(function(data){vm.book.title="",vm.getBooks()})["catch"](function(data){console.log("error adding book")})},vm.deleteBook=function(id){var bookId=vm.yourBooks[id]._id;Book.deleteUserBook(bookId).then(function(data){console.log("book deleted"),vm.getBooks()})["catch"](function(data){console.log("failed to delete book")})},vm.getBooks()}])},function(module,exports){angular.module("navbarCtrl",["satellizer"]).controller("navbarController",["$auth","$location",function($auth,$location){var vm=this;vm.logOut=function(){$auth.logout().then(function(){$location.path("/")})}}])},function(module,exports){angular.module("usersCtrl",["usersService","accountService"]).controller("usersController",["User","Account","$stateParams","$location","$anchorScroll",function(User,Account,$stateParams,$location,$anchorScroll){function searchRequestedBook(books,id){for(var i=0;i<books.length;++i)if(books[i]._id===id)return books[i]}var vm=this,bookId=$stateParams.book_id,currentUserName=null;vm.userData={},vm.requestedBook={},vm.swapTitle=null,vm.modalShown=!1,vm.currentUserInfo=function(){Account.getAccountInfo().then(function(data){currentUserName=data[0].name})["catch"](function(data){console.log("error....")})},vm.getUserInfo=function(){User.getUserInfo($stateParams.id).then(function(data){data.logged_in_user===!0?$location.path("/account"):(vm.userData=data[0],vm.requestedBook=searchRequestedBook(vm.userData.books,bookId))})["catch"](function(data){console.log("error...")})},vm.setRequestedBook=function(index){var book_id=vm.userData.books[index]._id;vm.requestedBook=searchRequestedBook(vm.userData.books,book_id),$location.hash("requested"),$anchorScroll()},vm.sendBookRequest=function(){""!=vm.swapTitle&&null!=vm.swapTitle&&User.createRequest($stateParams.id,vm.userData.name,currentUserName,vm.requestedBook.title,vm.swapTitle).then(function(data){vm.modalShown=!1,vm.swapTitle="",console.log("Success!!")})["catch"](function(data){vm.modalShown=!1,vm.swapTitle="",console.log("error...")})},vm.toggleModal=function(){vm.modalShown=!vm.modalShown},vm.getUserInfo(),vm.currentUserInfo()}])},function(module,exports){angular.module("bookDirective",[]).directive("bookDirective",[function(){return{restrict:"E",templateUrl:"app/views/partials/book.html",replace:!0,scope:{book:"="}}}])},function(module,exports){angular.module("modalDialogDirective",[]).directive("modalDialog",[function(){return{restrict:"E",scope:{show:"="},replace:!0,templateUrl:"app/views/partials/modalView.html",transclude:!0,link:function(scope,element,attrs){scope.dialogStyle={},attrs.width&&(scope.dialogStyle.width=attrs.width),attrs.height&&(scope.dialogStyle.height=attrs.height),scope.hideModal=function(){scope.show=!1}}}}])},function(module,exports){angular.module("navbarButtonDirective",[]).directive("navbarToggle",[function(){return{restrict:"E",template:'<div><span class="main-hamburger fa fa-bars"></span><div>',replace:!0,link:function(scope,element,attrs){var mainView=element.parent().parent().parent(),mainDiv=element.parent().parent(),nav=mainView[0].querySelector(".drawer-nav");element.on("click",function(e){nav.classList.toggle("js-drawer-nav-open"),e.stopPropagation()}),mainDiv.on("click",function(){nav.classList.remove("js-drawer-nav-open")})}}}])},function(module,exports){angular.module("accountService",[]).factory("Account",["$q","$http",function($q,$http){function getAccountInfo(){var deffered=$q.defer();return $http.get("/api/account").success(function(data){deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}function updateInfo(name,email,location,password){var deffered=$q.defer();return $http.put("/api/account",{name:name,email:email,location:location,password:password}).success(function(data){deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}function deleteAccount(){var deffered=$q.defer();return $http["delete"]("/api/account").success(function(data){deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}var accountService={getAccountInfo:getAccountInfo,updateInfo:updateInfo,deleteAccount:deleteAccount};return accountService}])},function(module,exports){angular.module("inboxService",[]).factory("Inbox",["$q","$http",function($q,$http){function getRequests(){var deffered=$q.defer();return $http.get("/api/inbox").success(function(data){deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}function setApproval(id){var deffered=$q.defer();return $http.put("/api/inbox",{bookRequestId:id}).success(function(data){deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}function deleteRequest(id){var deffered=$q.defer();return $http["delete"]("/api/inbox/"+id).success(function(data){deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}var inboxService={getRequests:getRequests,setApproval:setApproval,deleteRequest:deleteRequest};return inboxService}])},function(module,exports){angular.module("usersService",[]).factory("User",["$q","$http",function($q,$http){function getUserInfo(id){var deffered=$q.defer();return $http.get("/api/user/"+id).success(function(data){deffered.resolve(data)}).error(function(data){deffered.reject()}),deffered.promise}function createRequest(id,sendTo,fromUser,title,swapFor){var deffered=$q.defer();return $http.post("/api/user/"+id,{sendToUserId:id,sendTo:sendTo,fromUser:fromUser,title:title,swapFor:swapFor}).success(function(data){deffered.resolve(data)}).error(function(data){deffered.reject(data)}),deffered.promise}var userService={getUserInfo:getUserInfo,createRequest:createRequest};return userService}])},function(module,exports){angular.module("md5",[]).constant("md5",function(){function a(a){return c(b(d(a)))}function b(a){return f(g(e(a),8*a.length))}function c(a){try{}catch(b){o=0}for(var c,d=o?"0123456789ABCDEF":"0123456789abcdef",e="",f=0;f<a.length;f++)c=a.charCodeAt(f),e+=d.charAt(c>>>4&15)+d.charAt(15&c);return e}function d(a){for(var b,c,d="",e=-1;++e<a.length;)b=a.charCodeAt(e),c=e+1<a.length?a.charCodeAt(e+1):0,b>=55296&&56319>=b&&c>=56320&&57343>=c&&(b=65536+((1023&b)<<10)+(1023&c),e++),127>=b?d+=String.fromCharCode(b):2047>=b?d+=String.fromCharCode(192|b>>>6&31,128|63&b):65535>=b?d+=String.fromCharCode(224|b>>>12&15,128|b>>>6&63,128|63&b):2097151>=b&&(d+=String.fromCharCode(240|b>>>18&7,128|b>>>12&63,128|b>>>6&63,128|63&b));return d}function e(a){for(var b=Array(a.length>>2),c=0;c<b.length;c++)b[c]=0;for(var c=0;c<8*a.length;c+=8)b[c>>5]|=(255&a.charCodeAt(c/8))<<c%32;return b}function f(a){for(var b="",c=0;c<32*a.length;c+=8)b+=String.fromCharCode(a[c>>5]>>>c%32&255);return b}function g(a,b){a[b>>5]|=128<<b%32,a[(b+64>>>9<<4)+14]=b;for(var c=1732584193,d=-271733879,e=-1732584194,f=271733878,g=0;g<a.length;g+=16){var h=c,n=d,o=e,p=f;c=i(c,d,e,f,a[g+0],7,-680876936),f=i(f,c,d,e,a[g+1],12,-389564586),e=i(e,f,c,d,a[g+2],17,606105819),d=i(d,e,f,c,a[g+3],22,-1044525330),c=i(c,d,e,f,a[g+4],7,-176418897),f=i(f,c,d,e,a[g+5],12,1200080426),e=i(e,f,c,d,a[g+6],17,-1473231341),d=i(d,e,f,c,a[g+7],22,-45705983),c=i(c,d,e,f,a[g+8],7,1770035416),f=i(f,c,d,e,a[g+9],12,-1958414417),e=i(e,f,c,d,a[g+10],17,-42063),d=i(d,e,f,c,a[g+11],22,-1990404162),c=i(c,d,e,f,a[g+12],7,1804603682),f=i(f,c,d,e,a[g+13],12,-40341101),e=i(e,f,c,d,a[g+14],17,-1502002290),d=i(d,e,f,c,a[g+15],22,1236535329),c=j(c,d,e,f,a[g+1],5,-165796510),f=j(f,c,d,e,a[g+6],9,-1069501632),e=j(e,f,c,d,a[g+11],14,643717713),d=j(d,e,f,c,a[g+0],20,-373897302),c=j(c,d,e,f,a[g+5],5,-701558691),f=j(f,c,d,e,a[g+10],9,38016083),e=j(e,f,c,d,a[g+15],14,-660478335),d=j(d,e,f,c,a[g+4],20,-405537848),c=j(c,d,e,f,a[g+9],5,568446438),f=j(f,c,d,e,a[g+14],9,-1019803690),e=j(e,f,c,d,a[g+3],14,-187363961),d=j(d,e,f,c,a[g+8],20,1163531501),c=j(c,d,e,f,a[g+13],5,-1444681467),f=j(f,c,d,e,a[g+2],9,-51403784),e=j(e,f,c,d,a[g+7],14,1735328473),d=j(d,e,f,c,a[g+12],20,-1926607734),c=k(c,d,e,f,a[g+5],4,-378558),f=k(f,c,d,e,a[g+8],11,-2022574463),e=k(e,f,c,d,a[g+11],16,1839030562),d=k(d,e,f,c,a[g+14],23,-35309556),c=k(c,d,e,f,a[g+1],4,-1530992060),f=k(f,c,d,e,a[g+4],11,1272893353),e=k(e,f,c,d,a[g+7],16,-155497632),d=k(d,e,f,c,a[g+10],23,-1094730640),c=k(c,d,e,f,a[g+13],4,681279174),f=k(f,c,d,e,a[g+0],11,-358537222),e=k(e,f,c,d,a[g+3],16,-722521979),d=k(d,e,f,c,a[g+6],23,76029189),c=k(c,d,e,f,a[g+9],4,-640364487),f=k(f,c,d,e,a[g+12],11,-421815835),e=k(e,f,c,d,a[g+15],16,530742520),d=k(d,e,f,c,a[g+2],23,-995338651),c=l(c,d,e,f,a[g+0],6,-198630844),f=l(f,c,d,e,a[g+7],10,1126891415),e=l(e,f,c,d,a[g+14],15,-1416354905),d=l(d,e,f,c,a[g+5],21,-57434055),c=l(c,d,e,f,a[g+12],6,1700485571),f=l(f,c,d,e,a[g+3],10,-1894986606),e=l(e,f,c,d,a[g+10],15,-1051523),d=l(d,e,f,c,a[g+1],21,-2054922799),c=l(c,d,e,f,a[g+8],6,1873313359),f=l(f,c,d,e,a[g+15],10,-30611744),e=l(e,f,c,d,a[g+6],15,-1560198380),d=l(d,e,f,c,a[g+13],21,1309151649),c=l(c,d,e,f,a[g+4],6,-145523070),f=l(f,c,d,e,a[g+11],10,-1120210379),e=l(e,f,c,d,a[g+2],15,718787259),d=l(d,e,f,c,a[g+9],21,-343485551),c=m(c,h),d=m(d,n),e=m(e,o),f=m(f,p)}return Array(c,d,e,f)}function h(a,b,c,d,e,f){return m(n(m(m(b,a),m(d,f)),e),c)}function i(a,b,c,d,e,f,g){return h(b&c|~b&d,a,b,e,f,g)}function j(a,b,c,d,e,f,g){return h(b&d|c&~d,a,b,e,f,g)}function k(a,b,c,d,e,f,g){return h(b^c^d,a,b,e,f,g)}function l(a,b,c,d,e,f,g){return h(c^(b|~d),a,b,e,f,g)}function m(a,b){var c=(65535&a)+(65535&b),d=(a>>16)+(b>>16)+(c>>16);return d<<16|65535&c}function n(a,b){return a<<b|a>>>32-b}var o=0;return a}()),function(){var a;a=function(a){return["gravatarService",function(b){var c;return c=function(a,b){var c,d,e;d={};for(c in b)e=b[c],0===c.indexOf(a)&&(c=c.substr(a.length).toLowerCase(),c.length>0&&(d[c]=e));return d},{restrict:"A",link:function(d,e,f){var g,h,i,j;g=a?"gravatarSrcOnce":"gravatarSrc",h=f[g],delete f[g],i=c("gravatar",f),j=d.$watch(h,function(c){if(a){if(null==c)return;j()}e.attr("src",b.url(c,i))})}}}]},angular.module("ui.gravatar",["md5"]).provider("gravatarService",["md5",function(a){var b,c,d;return c=this,b=/^[0-9a-f]{32}$/i,d=function(a){var b,c,d;c=[];for(b in a)d=a[b],c.push(""+b+"="+encodeURIComponent(d));return c.join("&")},this.defaults={},this.secure=!1,this.protocol=null,this.urlFunc=function(c){var e,f,g,h,i;return g=c.protocol?c.protocol+":":"",i=c.secure?"https://secure":g+"//www",h=b.test(c.src)?c.src:a(c.src),f=[i,".gravatar.com/avatar/",h],e=d(c.params),e.length>0&&f.push("?"+e),f.join("")},this.$get=[function(){return{url:function(a,b){return null==a&&(a=""),null==b&&(b={}),c.urlFunc({params:angular.extend(angular.copy(c.defaults),b),protocol:c.protocol,secure:c.secure,src:a})}}}],this}]).directive("gravatarSrc",a()).directive("gravatarSrcOnce",a(!0))}.call(this)},function(module,exports){!function(A,d,B){"use strict";function l(){return["$animate",function(v){return{restrict:"AE",transclude:"element",priority:1,terminal:!0,require:"^^ngMessages",link:function(n,r,a,b,m){var f,k=r[0],p=a.ngMessage||a.when;a=a.ngMessageExp||a.whenExp;var d=function(a){f=a?w(a)?a:a.split(/[\s,]+/):null,b.reRender()};a?(d(n.$eval(a)),n.$watchCollection(a,d)):d(p);var e,q;b.register(k,q={test:function(a){var g=f;return a=g?w(g)?0<=g.indexOf(a):g.hasOwnProperty(a):void 0},attach:function(){e||m(n,function(a){v.enter(a,null,r),e=a;var g=e.$$attachId=b.getAttachId();e.on("$destroy",function(){e&&e.$$attachId===g&&(b.deregister(k),q.detach())})})},detach:function(){if(e){var a=e;e=null,v.leave(a)}}})}}}]}var w=d.isArray,x=d.forEach,y=d.isString,z=d.element;d.module("ngMessages",[]).directive("ngMessages",["$animate",function(d){function n(a,b){return y(b)&&0===b.length||r(a.$eval(b))}function r(a){return y(a)?a.length:!!a}return{require:"ngMessages",restrict:"AE",controller:["$element","$scope","$attrs",function(a,b,m){function k(a,b){for(var c=b,f=[];c&&c!==a;){var h=c.$$ngMessageNode;if(h&&h.length)return e[h];c.childNodes.length&&-1==f.indexOf(c)?(f.push(c),c=c.childNodes[c.childNodes.length-1]):c=c.previousSibling||c.parentNode}}var f=this,p=0,l=0;this.getAttachId=function(){return l++};var q,s,e=this.messages={};this.render=function(g){g=g||{},q=!1,s=g;for(var e=n(b,m.ngMessagesMultiple)||n(b,m.multiple),c=[],k={},h=f.head,p=!1,l=0;null!=h;){l++;var t=h.message,u=!1;p||x(g,function(a,c){!u&&r(a)&&t.test(c)&&!k[c]&&(u=k[c]=!0,t.attach())}),u?p=!e:c.push(t),h=h.next}x(c,function(a){a.detach()}),c.length!==l?d.setClass(a,"ng-active","ng-inactive"):d.setClass(a,"ng-inactive","ng-active")},b.$watchCollection(m.ngMessages||m["for"],f.render),this.reRender=function(){q||(q=!0,b.$evalAsync(function(){q&&s&&f.render(s)}))},this.register=function(g,b){var c=p.toString();e[c]={message:b};var d=a[0],h=e[c];f.head?(d=k(d,g))?(h.next=d.next,d.next=h):(h.next=f.head,f.head=h):f.head=h,g.$$ngMessageNode=c,p++,f.reRender()},this.deregister=function(b){var d=b.$$ngMessageNode;delete b.$$ngMessageNode;var c=e[d];(b=k(a[0],b))?b.next=c.next:f.head=c.next,delete e[d],f.reRender()}}]}}]).directive("ngMessagesInclude",["$templateRequest","$document","$compile",function(d,n,l){return{restrict:"AE",require:"^^ngMessages",link:function(a,b,m){var k=m.ngMessagesInclude||m.src;d(k).then(function(d){l(d)(a,function(a){b.after(a),a=z(n[0].createComment(" ngMessagesInclude: "+k+" ")),b.after(a),b.remove()})})}}}]).directive("ngMessage",l()).directive("ngMessageExp",l())}(window,window.angular)},function(module,exports){/**
